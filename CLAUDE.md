...
| `TWILIO_ACCOUNT_SID`| no      | Twilio Account SID; SMS disabled unless all three are set     |
| `TWILIO_AUTH_TOKEN` | no      | Twilio Auth Token                                            |
| `TWILIO_FROM`       | no      | Twilio sender number in E.164 (e.g. `+1xxxxxxxxxx`)          |
...
via Twilio's REST API (`sendSms` in `server.js` — no extra npm dependency). Messages fire
on: **request received**, **accepted**, **denied**, and **cancelled**. To enable:

1. Create a Twilio account, buy an SMS-capable number, note the Account SID + Auth Token.
2. Set `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM` in the Render dashboard.

Until those are set, every booking still works — the SMS step is simply skipped.
